...
class Vertex:
    ...
    def get_projection(self, surface: pygame.Surface, offset: Vector3, rotation: Vector3, FOV: int):
        relative = (self.position + offset).rotate(rotation)
    ...
    def display(self, surface: pygame.Surface, offset: Vector3, rotation: Vector3, FOV: int):
        if (offset + self.position).rotate(rotation).z <= 0: return
...
class Triangle:
    ...
    def center(self, offset: Vector3, rotation: Vector3) -> Vector3:
        return reduce(lambda a, b: a + b, [(vert.position.rotate(rotation) + offset) for vert in self.vertices]) / 3

    def is_visible(self, screen_width: int, screen_height: int, offset: Vector3, rotation: Vector3) -> bool:
        return any(
            (vert.position + offset).rotate(rotation).z > 0
            for vert in self.vertices
        )

    def distance_from_camera(self, offset: Vector3) -> float:
        return self.center(offset, Vector3(0, 0, 0)).distance(offset)

    def display(self, surface: pygame.Surface, offset: Vector3, rotation: Vector3, FOV: int) -> None:
        normal = self.get_normal()
        camera_dir = -offset - self.center(offset, rotation)
